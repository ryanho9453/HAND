...
class ConvModel:
    ...
    def __build_model(self, data_in):
        filters = self.pmeter['conv_base_filters']
        ...
        with tf.variable_scope('dense_layer'):
            ...
            dense = tf.layers.dense(
                inputs=pool2_flat,
                units=self.pmeter['fc_neu_num'],
                activation=tf.nn.relu)
            dropout = tf.layers.dropout(
                inputs=dense, rate=self.pmeter['keep_prob'])

        # output layer
        with tf.variable_scope('logits_layer'):
            """ why the shape of softmax ?  the number of labels """
            logits = tf.layers.dense(
                inputs=dropout, units=self.pmeter['label_class'] * 4)
            logits = tf.reshape(logits, [-1, 4, self.pmeter['label_class']])
            pred = {
                "result": logits
            }
        return pred, logits
    ...
    def __build_train_op(self, logits, labels):
        ...
        with tf.name_scope('optimizer'):
            optimizer = tf.train.AdamOptimizer(
                learning_rate=self.pmeter['learn_rate'])
    ...
    def __get_conv_layer(self, data_in, filters, name,
        ...
        with tf.variable_scope(name):
            ...
            if drop:
                conv = tf.layers.dropout(conv, rate=self.pmeter['keep_prob'])
            return conv
