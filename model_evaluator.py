...
class ModelEvaler:
    def __init__(self, config):
        self.reader = DataReader(config['processing']['reader'])
        self.config = config['processing']['evaler']
...
if __name__ == "__main__":
    ...
    config_path = '%s/config.json' % os.path.dirname(__file__)
    with open(config_path, 'r') as f:
        config = json.load(f)

    evaler = ModelEvaler(config)
